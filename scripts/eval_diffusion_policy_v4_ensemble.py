...
@torch.no_grad()
def ddim_ensemble_sample(model, norm_state, phase_id, betas, act_mean, act_std, horizon, num_ensemble, num_steps=50):
    device=norm_state.device; seq_dim=horizon*4
    
    # Expand state and phase for batch processing
    ...
    # Generate initial noise for the entire ensemble
    ...
    # Denormalize and average the ensemble
    denorm = x * act_std + act_mean
    avg_plan = torch.mean(denorm, dim=0)
    
    acts = avg_plan.view(horizon,4)
    return torch.clamp(acts,-1,1).cpu().numpy()

def run(a):
    device=torch.device("mps" if torch.backends.mps.is_available() else "cpu")
    ckpt=torch.load(a.checkpoint,map_location=device)
    model=CondDiffusionNetV4(ckpt["seq_dim"],ckpt["state_dim"],model_dim=ckpt.get("hidden_dim",512),depth=ckpt.get("depth",4))
    ...
    for ep in range(a.episodes):
        ...
        while True:
            ...
            if len(plan)==0:
                ...
                seq=ddim_ensemble_sample(model,norm_state,phase_id,betas,act_mean,act_std,horizon,a.num_ensemble,num_steps=a.num_steps)
                plan=seq.tolist()
    ...
    env.close()
    print(f"DDIM Ensemble SR: {successes/a.episodes*100:.2f}%")
...
if __name__=="__main__":
    ...
    ap.add_argument("--num_ensemble",type=int,default=8)
