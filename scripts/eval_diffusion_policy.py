...
@torch.no_grad()
def sample_sequence(model, seq_dim, timesteps, device, step_scale=0.1):
...
def run(a):
    ...
    seq_dim = meta["seq_dim"]; timesteps = meta["timesteps"]
    ...
    for ep in range(a.episodes):
        ...
        while True:
            if steps % horizon ==0:
                seq = sample_sequence(model, seq_dim, timesteps, device, step_scale=a.step_scale)
...
if __name__=="__main__":
    ...
    ap.add_argument("--step_scale", type=float, default=0.1)
    a=ap.parse_args()
    run(a)
