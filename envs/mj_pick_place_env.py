...
class MjPickPlaceEnv:
    ...
    def _stabilize(self):
        # Clamp cube free joint velocities to avoid integrator blowups
        vadr = self.model.jnt_dofadr[self.cube_joint_id]
        qv = self.data.qvel[vadr:vadr+6]
        if not (qv==qv).all():  # NaN check
            self.data.qvel[vadr:vadr+6] = 0
        else:
            norm = (qv**2).sum()**0.5
            if norm > 5.0:
                self.data.qvel[vadr:vadr+6] *= 5.0 / (norm + 1e-9)
